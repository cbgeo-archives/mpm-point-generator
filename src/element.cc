...
double Element::calculate_volume() {
  ...
  // Get number of vertices in one element
  const unsigned nvertices = coordinates_.size();
    ...
    */
    ...
    if (nvertices == 8) {
      ...
      */
      ...
      // Based on the first vertex
      const Eigen::Vector3d a(coordinates_.at(0)[0], coordinates_.at(0)[1],
                              coordinates_.at(0)[2]),
          b(coordinates_.at(1)[0], coordinates_.at(1)[1], coordinates_.at(1)[2]),
          c(coordinates_.at(2)[0], coordinates_.at(2)[1], coordinates_.at(2)[2]),
          d(coordinates_.at(3)[0], coordinates_.at(3)[1], coordinates_.at(3)[2]);
  
  
      volume = 1.0 / 6 * (d - a).dot((b - a).cross(c - a));
